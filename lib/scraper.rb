...
class Scraper
  ...
  def self.scrape_base_classes
    ...
    #The drop down menu on the main screen that has all the classes
    ...
    classes.each do |player_class|
      ...
      #The sub menu for each class that contains each specialization
      player_class.css("ul li").each do |specialization|
        ...
        #Each key value pair in the spec hash is the specialization name as the key
        #and the url to the specialization page as the value
        spec_hash[spec_name.to_sym] = spec_url
      end

      #Specialization hash is added to the class hash under the :specialization symbol
